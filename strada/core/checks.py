"""
Data-quality check functions for STRADA datasets.

Generic checks (G1–G6) apply to any STRADA analysis; cycling-specific checks
(C1–C3) are relevant only when the dataset is filtered to cycling /
...
The registry that wires these functions to their metadata (id, name,
description, severity, category, family) lives in ``strada.core.verify``.
"""
...
# ─────────────────────────────────────────────────────────────────────────────
#  Severity (canonical source — verify.py re-exports this as CHECK_SEVERITY)
# ─────────────────────────────────────────────────────────────────────────────
#
# Sub-check IDs (e.g. "G2.1") inherit severity from their parent ("G2") via
...
def _status_for(check_id: str, issue_count: int) -> str:
    """Map (check_id, issue_count) → display status.

    Returns ``"pass"`` when the check found no issues; otherwise ``"critical"``
    if the parent check is critical-severity, ``"warning"`` otherwise.
    """
    if issue_count == 0:
        return "pass"
...
def _empty(series: pd.Series) -> pd.Series:
    """Boolean mask of NaN-or-whitespace values."""
...
def check_g2_crash_type(
    ...
    # --- G2.1 — missing values ---
    missing_o = df_olyckor[_empty(df_olyckor[COL_CRASH_TYPE])]
    missing_p = df_personer[_empty(df_personer[COL_CRASH_TYPE])]
...
def check_g3_road_user_category(
    ...
    # Effective category: P if available, else S
